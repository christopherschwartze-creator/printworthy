...
the module this lane owns. scipy (a printworthy dependency) backs the min-cut. The
...
try:
    _COACD_BUDGET_S = float(os.environ.get("PRINTWORTHY_COACD_BUDGET_S", "60") or 60)
except Exception:
...
def _bed_mm(profile):
    ...
    Accepts a profile NAME (resolved via printworthy.profiles, lazily), a dict,
    ...
    try:
        if isinstance(profile, str):
            label = profile
            try:
                from printworthy.profiles import get_profile  # product layer, lazy
...
def _run_coacd_subprocess(v, f):
    ...
    import sys
    import tempfile
    tmpdir = tempfile.mkdtemp(prefix="printworthy_coacd_")
    ...
    try:
        ...
        code = ("import sys, printworthy.split as s; "
...
def _coacd_reason_text(reason):
    ...
    return {
        "unavailable": ("CoACD is not installed - install the extra "
                        "`printworthy[split]` to enable bed-splitting"),
