"""printworthy.slicer — external slicer CLI wrapper: the "saves X g / Y min" number.
...
out of the g-code footer comments. That turns printworthy's geometry work
...
def _profile_flags(name, profile):
    """Map the (optional) printworthy profile dict to slicer CLI flags.
...
def estimate(model_path, profile=None, slicer=None,
    ...
    try:
        ...
        # -- slice ---------------------------------------------------------
        work = out_dir or tempfile.mkdtemp(prefix="printworthy_slice_")
