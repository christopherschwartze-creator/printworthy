"""printworthy CLI — ``printworthy <check|fix|prep|reinforce|calibrate|supports|app>
...``.

One console entry (`printworthy = printworthy.cli:main` in pyproject). The mesh
subcommands are thin wrappers over :func:`printworthy.pipeline.prep`; `reinforce`,
    ...
    if full:
        ...
        p.add_argument("--supports", action="store_true",
                       help="also emit a risk-driven SupportEnforcer/"
                            ...
                            "(run `printworthy supports -h` for the standalone "
...
def _run_supports(argv):
    """``printworthy supports`` -- standalone dispatch (own arg surface, like
...
def main(argv=None):
    ...
    try:
        ...
        from .core._mesh_util import say
        say(f"FAIL: printworthy could not process this file "
