...
* `scale_to_fit(mesh, profile)` -> does-it-fit + suggested uniform scale.
  ...
  mesh is in mm (the house unit convention everywhere in printworthy).

Pure stdlib + (lazy) numpy. Never raises from any public entry point.
Run as a script for the self-tests:  python -m printworthy.profiles
"""
...
#  pipeline is running and what it may spend. Applied by printworthy.pipeline.
# ---------------------------------------------------------------------------
PREP_PRESETS = {
    "space": {
