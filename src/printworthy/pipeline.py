"""printworthy.pipeline — the ONE entry point.

    prep(path_or_mesh, ...) -> PrepResult dict

Orchestrates the vendored, validated engines in `printworthy.core`:
...
printworthy.profiles.get_preset. `preset="space"` is the hosted-demo bundle:
...
def prep(path_or_mesh, *, profile="generic_fdm", material=None, mode=None,
    ...
    profile          : printer preset name / dict (printworthy.profiles).
    ...
    fem_warp         : opt-in inherent-strain warp FEM (uncalibrated unless a
                       coupon was fitted via `printworthy calibrate`).
    ...
    trust_camera     : opt-in GEOMETRIC VISIBILITY ("trust") map (core/
                       ...
                       coordinate frame (computed BEFORE printworthy's internal
    ...
    try:
        ...
        plain = ("Sorry -- printworthy hit an internal error and could not finish. "
        ...
        return {"ok": False, "verdict": "ERROR",
                ...
                "report": {"markdown": f"# printworthy report -- ERROR\n\n{plain}\n\n"
                                       f"_details: {msg}_\n",
...
def _prep(path_or_mesh, *, profile, material, mode, preset, fix, orient,
    ...
    if out_dir is None:
        out_dir = tempfile.mkdtemp(prefix="printworthy_")
    ...
    try:
        _probe = os.path.join(out_dir, ".printworthy_write_probe")
    ...
    # as-uploaded/welded mesh, BEFORE printworthy's own print-size rescale below
    ...
    if fit:
        ...
        if fit.get("fits") is False:
            notes.append("part exceeds the bed -- scale by "
                         ...
                         "(printworthy.split.split_for_bed)")
