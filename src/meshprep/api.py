...
FastAPI is intentionally NOT a dependency of meshprep (default install stays
...
Usage:
    uvicorn "meshprep.api:create_app" --factory --host 127.0.0.1
"""
...
def create_app():
    ...
    absent (import-guarded: `import meshprep.api` never needs fastapi)."""
    try:
        from fastapi import FastAPI, File, UploadFile
        from fastapi.responses import JSONResponse
    except ImportError as e:
        raise RuntimeError(
            "meshprep.api is an optional stub and FastAPI is not installed. "
            "Run `pip install fastapi uvicorn` (it is deliberately not part "
            "of the default meshprep install)."
        ) from e

    from meshprep import __version__

    app = FastAPI(
        title="meshprep farm endpoint (STUB — NOT hardened)",
        version=__version__,
        description=__doc__,
    )
    ...
    def health():
        return {"ok": True, "version": __version__,
                "status": "stub — not hardened, see meshprep/api.py docstring"}
    ...
    async def check(file: UploadFile = File(...)):
        ...
        try:
            with tempfile.NamedTemporaryFile(suffix=suffix, delete=False) as fh:
                ...
                tmp = fh.name
            try:
                from meshprep.pipeline import prep  # lazy: product layer
