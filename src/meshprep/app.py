"""meshprep Gradio front door — the Hugging Face Space slice.
...
def _scrub(md, result):
    ...
    try:
        ...
        md = re.sub(r"[A-Za-z]:[\\/][^\s`'\"<>|)]*meshprep_[^\s`'\"<>|)]*",
                    "(your download package)", md)
        md = re.sub(r"/[^\s`'\"<>|)]*meshprep_[^\s`'\"<>|)]*",
                    "(your download package)", md)
...
def run(file, printer, resize, print_mm, unit, nozzle, material,
    ...
    try:  # optional Space-side face cap (keeps free-CPU jobs inside timeout)
        mf = int(os.environ.get("MESHPREP_MAX_FACES", "0"))
        if mf > 0:
            kw["max_faces"] = mf
    except (TypeError, ValueError):
        pass
    # hosting-context bundle (MESHPREP_PRESET=space on the hosted Space):
    ...
    preset = (os.environ.get("MESHPREP_PRESET") or "").strip() or None
    if preset:
        kw["preset"] = preset
