...
modules live alongside it in meshprep.core (vendored, package-relative).
    ...
    if check_animation:
        report["channels"]["deformation"] = {
            "note": "animation / rig-readiness checks ship in the separate "
                    "animation build (autorig); not bundled in meshprep"}
...
def build_cert(report: dict) -> dict:
    ...
    # no cert item (the rig-readiness checks are not bundled in meshprep).
