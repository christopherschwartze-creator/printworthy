...
Design constraints (deliberate):
  ...
  * `meshprep.pipeline.prep` is imported LAZILY inside the function, so this
...
def batch_prep(folder_in, folder_out, **prep_kwargs):
    """Run ``meshprep.prep`` on every mesh file directly inside `folder_in`
    (non-recursive, sorted, sequential); write per-file JSON results and a
    ``summary.csv`` to `folder_out`. Never raises."""
    try:
        try:
            from meshprep.pipeline import prep  # lazy: may not exist yet
        except Exception as e:
            return {"ok": False, "n_total": 0, "n_ok": 0, "results": [],
                    "summary_csv": None,
                    "note": ("meshprep.pipeline is not available "
