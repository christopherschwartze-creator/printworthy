...
4. DETERMINISM. Same file + same rates -> byte-identical quote JSON
   ...
   audit. Include meshprep version + profile hash in "assumptions".
...
6. WIRE-IN: CLI `meshprep quote part.stl --rates rates.json` and
   `meshprep quote ./uploads/ --csv quotes.csv`.
...
_NOT_BUILT = (
    "print quoting is a documented stub -- build plan in meshprep/quote.py. "
